...
class CheckboxTree extends React.Component {
  ...
  componentWillReceiveProps({ nodes, checked, expanded }) {
    if (!isEqual(this.props.nodes, nodes) || this.props.dynamicUpdate) {
  ...
  getFormattedNodes(nodes, parent) {
    return nodes.map(node => {
      ...
      if (Array.isArray(node.children) && node.children.length > 0) {
        formatted.children = this.getFormattedNodes(formatted.children, { parent: parent, key: formatted.value });
      } else {
  ...
  renderTreeNodes(nodes, parent = {}) {
    const { disabled, expandDisabled, noCascade, onlyLeafCheckboxes, optimisticToggle, showNodeIcon } = this.props;
